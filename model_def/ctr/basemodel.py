# Taken from https://github.com/yuangh-x/2022-NIPS-Tenrec/blob/master/model/ctr/basemodel.py
"""
Author:
...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
            validation_data=None, shuffle=False, callbacks=None):
        """
        ...
        if do_validation is True:
            print(f"CTR loaded Validation dataset size: {len(val_x)}")
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            total_loss_epoch = 0
            train_result = {}
            for (x_train, y_train) in tqdm(train_loader, total=len(train_loader), desc=f'Epoch: {epoch}, Training Batch'):
        ...
        return self.history, best_model
    ...
    def compile(self, args, optimizer,
                loss=None,
                metrics=None,
                ):
        """
    ...
    def _get_optim(self, optimizer):
        if isinstance(optimizer, str):
            if optimizer == "sgd":
                ...
                optim = torch.optim.Adam(self.parameters(), lr=self.args.lr)  # 0.001
            elif optimizer == "adagrad":
                optim = torch.optim.Adagrad(self.parameters(), lr=self.args.lr)  # 0.01
            elif optimizer == "rmsprop":
                optim = torch.optim.RMSprop(self.parameters(), lr=self.args.lr)
            else:
